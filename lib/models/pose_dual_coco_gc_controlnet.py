...
class PoseCotrain(nn.Module):
    ...
    def forward(self, x):
        # Training
        if type(x)==list:
            sup_x, controlnet_x, unsup_x = x

            batch_size = x[0].shape[0]
            sup_fea1, sup_ht1 = self.resnet(sup_x)
            sup_fea2, sup_ht2 = self.resnet2(sup_x)

            controlnet_fea1, controlnet_ht1 = self.resnet(controlnet_x)
            controlnet_fea2, controlnet_ht2 = self.resnet2(controlnet_x)
            ...
            return sup_ht1, sup_ht2, controlnet_ht1, controlnet_ht2, unsup_ht1, unsup_ht2, unsup_ht_trans1, unsup_ht_trans2, cons_ht1, cons_ht2, out_dic
